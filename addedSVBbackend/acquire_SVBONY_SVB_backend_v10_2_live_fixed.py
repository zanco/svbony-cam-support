...
def _v23_capture_one_buffer(buf, zbase, tbase, nx, ny, nz, conf_file, sequence, camera_serial, live):
    ...
    # With multiprocessing 'spawn', make libusb globally visible before importing
    # or loading either camera SDK path in the worker.
    try:
        ctypes.CDLL("libusb-1.0.so", mode=ctypes.RTLD_GLOBAL)
    except Exception:
        ctypes.CDLL("/home/hacker3/svbony/SVBCameraSDK/lib/x64/libusb-1.0.so", mode=ctypes.RTLD_GLOBAL)

    # Do not use the native SVB capture API in this worker.
    # PySVB is the sole owner of camera open/start/capture/stop/close and frame delivery.

    z = np.ctypeslib.as_array(zbase.get_obj()).reshape(ny, nx, nz)
    t = np.ctypeslib.as_array(tbase.get_obj())

    # Read exposure and gain from the same svbconfiguration.ini used by STVID.
    # This keeps observing parameters out of the Python source code; exposure is in us.
    # this Python program. Exposure is in microseconds, as in svbconfiguration.ini.
    cfg = configparser.ConfigParser(inline_comment_prefixes=("#", ";"))
    if not cfg.read(conf_file):
        raise RuntimeError("V10: could not read config file: %s" % conf_file)
    ...
    logger.info(
        "V10 CONFIG: %s exposure=%d us gain=%d",
    ...
    try:
        ...
        # This bridge does not provide the real ASI_* constants. Camera settings have
        # already been applied from svbconfiguration.ini by the bridge itself.
        # Do not try to call ASI_GAIN/ASI_EXPOSURE/ASI_IMG_RAW8 here: those belong
        # to the real ZWO/ASI backend, not to this SVBONY compatibility bridge.
        camera.start_video_capture()

        logger.info("V10 WORKER START: buffer=%d sequence=%d", buf, sequence)
        ...
        for i in range(nz):
            ...
            for attempt in range(6):
                ...
                frame_ok = True
                if attempt:
                    logger.info("V10 worker buffer %d: recovered frame %d after %d retries", buf, i, attempt)
                break
            if not frame_ok:
                raise RuntimeError("V10: no valid non-black frame after 6 attempts at frame %d" % i)
            ...
            if (i + 1) % 10 == 0:
                rss_now, rss_peak = _v23_mem_mb()
                logger.debug("V10 WORKER MEMORY: buffer=%d frame=%d rss_now=%.1f MiB rss_peak=%.1f MiB",
                             buf, i + 1, rss_now, rss_peak)

        logger.info("V10 WORKER COMPLETE: buffer=%d frames=%d elapsed=%.3f sec",
                    buf, nz, time.time() - batch_start)
    except Exception as e:
        logger.error("V10 WORKER ERROR buffer=%d: %s: %s", buf, type(e).__name__, e)
        logger.error("V10 WORKER TRACEBACK:\n%s", traceback.format_exc())
        raise
    finally:
        try:
        ...
        _v23_malloc_trim()
        rss_now, rss_peak = _v23_mem_mb()
        logger.info("V10 WORKER EXIT: buffer=%d rss_now=%.1f MiB peak=%.1f MiB",
...
def capture_asi(free_queue, ready_queue, z1base, t1base, z2base, t2base, nx, ny, nz, tend, device_id, live, conf_file, camera_serial):
    ...
    logger.info("V10 CAPTURE START: now=%.3f tend=%.3f remaining=%.3f sec",
    ...
    try:
        while time.time() < tend:
            buf = free_queue.get()
            buffer_sequence += 1
            logger.debug("V10 GOT FREE BUFFER %d; starting isolated SDK worker sequence=%d", buf, buffer_sequence)
            ...
            worker = multiprocessing.Process(
                ...
                args=(buf, zbase, tbase, nx, ny, nz, conf_file, buffer_sequence, camera_serial, live)
            )
            worker.start()
            worker.join()

            logger.info("V10 WORKER JOIN: buffer=%d pid=%s exitcode=%s",
                        buf, worker.pid, worker.exitcode)
            if worker.exitcode != 0:
                raise RuntimeError("V10 SVBONY buffer worker failed: buffer=%d exitcode=%s" %
                                   (buf, worker.exitcode))

            total_frames += nz
            ready_queue.put(buf)
            logger.debug("V10 READY: buffer=%d total_buffers=%d total_frames=%d",
        ...
        logger.error("V10 CAPTURE UNHANDLED EXCEPTION: %s", reason)
        logger.error("V10 CAPTURE TRACEBACK:\n%s", traceback.format_exc())
    finally:
        rss_now, rss_peak = _v23_mem_mb()
        logger.info("V10 CAPTURE EXIT: reason=%s elapsed=%.3f sec buffers=%d frames=%d",
                    reason, time.time() - capture_start, buffer_sequence, total_frames)
        logger.info("V10 CAPTURE FINAL RSS: now=%.1f MiB peak=%.1f MiB", rss_now, rss_peak)
        logger.info("Capture: %s - Exiting", reason)
